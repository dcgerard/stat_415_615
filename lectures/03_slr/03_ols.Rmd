...
- To simulate $Y_i$'s, we generate new errors each replication.
    ...
    ```{r}
    epsilon <- runif(n = 100, min = -0.4, max = 0.4)
...
- We can visualize the variability about the estimates
    ...
    ```{r}
    qplot(beta0_hat, bins = 30) + geom_vline(xintercept = beta0, color = "red")
    
    qplot(beta1_hat, bins = 30) + geom_vline(xintercept = beta1, color = "red")
    ```
...
- But remember that $\hat{\beta}_1$ and $\hat{\beta}_2$ minimize the objective, which is exactly the MSE. That means that
    ...
    s_2^2 < s_1^2
    $$
- Dividing by a smaller number ($n-2$) instead of a $n$ corrects for this. That's why we use
...
- Note that, given the normal assumption and values of $\beta_0$, $\beta_1$, and $\sigma^2$, we can calculate the density (which roughly means the probability) of our observed sample $(X_1, Y_1),(X_2, Y_2),\ldots, (X_n, Y_n)$.
...
- Typically, we actually worth with the log-likelihood
    $$
...
- The MLE of $\sigma^2$ is $\frac{n}{n-2}\hat{\sigma}^2$ (the uncorrected standard deviation estimate).
