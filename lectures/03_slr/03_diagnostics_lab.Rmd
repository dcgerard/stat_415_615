...
2. Is an F-test lack-of-fit appropriate here? If so, un and F-test lack of fit and state your conclusions.
...
a_1 <- augment(lm_1)
qplot(x = .fitted, y = .resid, data = a_1) +
  geom_hline(yintercept = 0)
```
...
a_2 <- augment(lm_2)
qplot(x = .fitted, y = .resid, data = a_2) +
  geom_hline(yintercept = 0)
```
...
a_3 <- augment(lm_3)
qplot(x = .fitted, y = .resid, data = a_3) +
  geom_hline(yintercept = 0)
```
...
```{r}
qplot(sample = .resid, data = a_3, geom = "qq") +
  geom_qq_line()
```
...
We estimate a power-law growth between PSA and Volume ($p < 0.001$, $n = 97$), with an estimated relationship of $\text{psa} = 4.5 \times \text{Volume}^{0.72}$. What this means is that cancers that are twice the volume tend to have a PSA's about 64\% larger (since $2^{0.72} = 1.645$), with a 95\% confidence interval of around 50\% to 81\% larger.
