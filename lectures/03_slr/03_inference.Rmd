...
- A full interpretation might go something like this:
    ...
    > We estimate that incumbants to have on average 3.1 percentage-points higher vote-share in years which experience 1 percentage-point more growth (95\% confidence interval of 1.6 PP higher to 4.6 PP higher). We have strong evidence that this association is not due to chance alone ($p = 0.00061$, $n = 16$).
...
- But $\beta_0$, $\beta_1$, and $\sigma^2$ are not known. So if we just added and subtracted the 2 MSE from the estimated regression line, we could make some big mistakes if our regression line estimate was way off.
    ...
    ggplot() +
      geom_line(data = dfline, mapping = aes(x = x, y = y, color = Line), linewidth = 2) +
      geom_errorbar(data = df, mapping = aes(x = x, ymin = y, ymax = yend), width = 0.05, linewidth = 2) +
      theme_classic() +
      theme(axis.text = element_blank(),
            axis.title = element_blank(),
            axis.ticks = element_blank()) +
...
ggplot(data = hibbs, mapping = aes(x = growth, y = vote)) +
  geom_point() +
  geom_line(data = newdf, mapping = aes(x = growth, y = Fit), lty = 1, col = "black", linewidth = 1, alpha = 1/2) +
  geom_line(data = banddf, mapping = aes(x = growth, y = Value, group = Group, color = Type), linewidth = 1) +
  scale_color_colorblind() +
  xlab("Growth") +
  ylab("Vote")
```
...
- Sum of squares of reduced model
    ...
    ggplot(data = hibbs) +
      ...
      geom_line(aes(x = growth, y = ybar), linewidth = 2, col = "blue", alpha = 1/3) +
      xlab("Growth") +
      ylab("Vote") +
...
- Sum of squares of full model
    ...
    ggplot(data = hibbs) +
      geom_point(aes(x = growth, y = vote)) +
      geom_line(aes(x = growth, y = yhat), linewidth = 2, col = "blue", alpha = 1/3) +
      xlab("Growth") +
      ylab("Vote") +
