...
- You can use the standard deviation of the variables to help guide you. If the standard deviation is between 1 and 10, then you have a good scale. But this is a rough rule of thumb.
...
- What if you are logging variables?
    ...
    - You can choose a good multiplier by seeing if the standard deviation of the log of that base is between 1 and 10 (but this is a rough rule of thumb).
...
- **Exercise**: A study was exploring the association between a country's wine consumption (liters per person per year) and mortality rate (deaths per 1000 individuals). Researchers found that a power-law relationship was a fine approximation. Based on the below plot alone, choose a good multiplier to interpret the coefficient of a regression of log-mortality on log-wine. Verify your result using our rule of thumb above.
    ...
    2 would be the best, as there aren't oo many countries with 10 times as much wine consumption.
    ```
...
- The idea behind using standard deviations is that roughly 68\% of observations will be within 1 standard deviation of the mean. So this difference represents something meaningful.
...
- If you do this to all variables in your model, you can use the following multiple regression model:
    $$
    Y_i = \beta_0 + \beta_1Z_{i1} + \beta_2 Z_{i2} + \cdots + \beta_{p-1}Z_{i, p-1} + \epsilon_i
    $$
